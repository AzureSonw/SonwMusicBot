...
public enum AudioSource
{
    ...
    APPLE_MUSIC(
        "applemusic",
        "Apple Music tracks, albums, playlists, and artists",
        15,
        (manager, config) -> {
            String mediaApiToken = config.getAppleMusicMediaApiToken();
            AppleMusicSourceManager appleMusic = new AppleMusicSourceManager(
                new String[] {"ytsearch:\"%ISRC%\"", "ytsearch:%QUERY%"},
                mediaApiToken == null || mediaApiToken.trim().isEmpty() ? null : mediaApiToken,
                config.getAppleMusicCountryCode(),
                unused -> manager
            );
            appleMusic.setPlaylistPageLimit(config.getMaxAppleMusicPlaylistPages());
            appleMusic.setAlbumPageLimit(config.getMaxAppleMusicAlbumPages());
            manager.registerSourceManager(appleMusic);
        }
    ),
    SOUNDCLOUD(
        "soundcloud",
    ...
    BILIBILI(
        "bilibili",
        "Bilibili videos, multi-part videos, audio playlists, favorites, seasons, and series",
        90,
        (manager, config) -> {
     ...
     * 
     * <p>OAuth mode still keeps the regular web/mobile clients available for
     * playback. Apple Music is resolved through LavaSrc mirror tracks, and those
     * mirrors can fail if only TV clients are allowed to provide stream formats.
     */
