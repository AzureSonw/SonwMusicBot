...
public class ForceRemoveCmd extends DJCommand
{
    ...
    private void removeAllEntries(User target, CommandEvent event)
    {
        ...
        if (count == 0)
        {
            event.replyWarning("**"+target.getName()+"** 没有加音乐到歌单里！");
        }
        else
        {
            event.replySuccess("移除了 "+FormatUtil.formatUsername(target)+" 的 `"+count+"` 首歌！");
        }
    }
}
