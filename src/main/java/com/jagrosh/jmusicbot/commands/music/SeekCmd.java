...
public class SeekCmd extends MusicCommand
{
    ...
    public void doCommand(CommandEvent event)
    {
        ...
        {
            event.replyError("调整不了 **" + playingTrack.getInfo().title + "** 去调你自己加的歌啦！");
            return;
        }
        ...
        if (seekTime == null)
        {
            event.replyError("输错了啦！Ex.: `1:02:23` `+1:10` `-90`, `1h10m`, `+90s`");
            return;
        }
        ...
        {
            event.replyError("调整不到 `" + TimeUtil.formatTime(seekMilliseconds) + "`，这歌都没那么长啊喂！");
            return;
        }
        ...
        {
            event.replyError("出bug了啦! bug: " + e.getMessage());
            LOG.warn("Failed to seek track " + playingTrack.getIdentifier(), e);
            return;
        }
        event.replySuccess("调整成功~ `" + TimeUtil.formatTime(playingTrack.getPosition()) + "/" + TimeUtil.formatTime(playingTrack.getDuration()) + "`!");
    }
}
